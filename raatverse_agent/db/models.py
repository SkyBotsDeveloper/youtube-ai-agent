...
class ScriptDraftRecord(Base):
    ...
    tts_narration_script: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class AudioAssetRecord(Base):
    ...
    tts_chunks_json: Mapped[list] = mapped_column(JSON, default=list)
    tts_quality_json: Mapped[dict | None] = mapped_column(JSON, nullable=True)
...
class VideoRenderRecord(Base):
    ...
    timing_report_json: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    error_message: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now)
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, onupdate=utc_now
